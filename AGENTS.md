...
- **Per-engine testing.** `SPORA_TEST_DB_DRIVER` switches the suite to a real engine: `sqlite` (default), `mysql`, or `mariadb`. With `mysql` or `mariadb`, every Pest parallel worker creates its own `spora_test_w<PID>_<RAND>` database on the configured server, installs the schema once, and drops the DB on exit. Set `SPORA_TEST_DB_HOST` / `_PORT` / `_USER` / `_PASSWORD` (defaults: 127.0.0.1:3306 root:root). Migration tests (`tests/Feature/Database/*`) issue DDL mid-test, so they call `TestDatabaseFactory::freshDatabase()` and pay one extra schema install per test for safety. The CI pipeline runs the suite against MySQL 9.7 and MariaDB 12.3 on every PR.

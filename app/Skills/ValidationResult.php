...
/**
 * Outcome of running {@see SkillValidator} against a parsed frontmatter.
 *
 * Carries two parallel arrays — `errors` (the skill cannot be used) and
 * `warnings` (the skill can be used, but with caveats surfaced to the
 * operator). Each entry is a free-form assoc with at minimum `code`,
 * `severity`, `message`. Optional `path` (JSON-pointer-ish) lets the UI
 * highlight the offending field.
 *
 * The class is mutable only via constructor and named methods so it can
 * be passed around safely across the scanner → controller → UI.
 */

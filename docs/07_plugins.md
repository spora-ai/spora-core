...
## Companion plugins (`suggest`)

Plugins can declare optional companion packages using Composer's standard
`suggest` field in `composer.json` — no new manifest field is invented. The
`PluginLoader` reads `composer.json`'s `suggest` at boot time and exposes the
list through `PluginLoader::suggestedPackages()`; `PluginsController`
serialises it into the plugin resource surfaced by `GET /api/v1/plugins`,
where the SPA's `PluginDetailDialog` renders a "Companion plugins" card
with a one-click install button.

```jsonc
// spora-plugin-minimax/composer.json
{
    "name": "spora-ai/spora-plugin-minimax",
    "type": "spora-plugin",
    "...": "...",
    "suggest": {
        "spora-ai/spora-plugin-media-archive": "Browse, filter, and download media generated by this plugin from the admin UI"
    }
}
```

`suggest` is informational — no semver enforcement, no auto-install, no hard
dependency. Hand-rolled plugins that don't ship a `composer.json` simply
have no `suggests` field; the UI section is hidden.

---

## Shipping a plugin admin UI

Plugins that want a dedicated admin panel (sidebar tile + mounted Vue app)
ship a pre-built Vue IIFE bundle alongside the PHP entry point. The
host SPA auto-discovers the bundle through the apps registry and mounts it
inside the shared `<RouterView>` slot — no extra routing work, no
rebuilding the host SPA.

### Repository layout

```
spora-plugin-media-archive/
├── plugin.json                  ← slug, class, optional frontendEntry
├── composer.json                ← suggest × companion plugins
├── src/                          ← PHP entry point
│   ├── MediaArchivePlugin.php
│   └── MediaArchiveApp.php
├── frontend/                     ← Vue sources + pre-built bundle
│   ├── package.json
│   ├── vite.config.ts            ← build.lib → iife, write to '.'
│   ├── index.html
│   ├── src/
│   │   ├── main.ts               ← defines window.SporaApp.{mount, unmount}
│   │   ├── App.vue
│   │   └── components/
│   ├── main.js                   ← committed pre-built IIFE bundle
│   └── style.css                 ← committed pre-built CSS
└── README.md
```

### `plugin.json` + `VueAppInterface`

The plugin's `plugin.json` declares the class and slug exactly as for a
tool-only plugin. Two ways to declare the UI entry point:

```json
{
    "slug": "media-archive",
    "class": "Spora\\Plugins\\MediaArchive\\MediaArchivePlugin",
    "description": "Browse, filter, and download media generated by your agents.",
    "icon": "image",
    "frontendEntry": "main.js"
}
```

…or implement `Spora\Apps\VueAppInterface` in PHP and return the entry path
from `entry()`. The `AppsController` serialises the resolved entry into the
`frontendEntry` field of `GET /api/v1/apps`, which the host SPA reads when
mounting `/apps/<slug>`.

```php
final class MediaArchiveApp implements VueAppInterface
{
    public function name(): string { return 'media-archive'; }
    public function displayName(): string { return 'Media Archive'; }
...
`frontend/vite.config.ts` is configured for IIFE format so the bundle is
loadable via a plain `<script>` tag from the host SPA. The relevant knobs:

```ts
build: {
    outDir: '.',                                // writes to frontend/ itself
    lib: {
        entry: 'src/main.ts',
        formats: ['iife'],
    ...
    rollupOptions: {
        external: ['vue', 'pinia'],              // shared with the host SPA
    },
}
```

Vue and Pinia are externalised so reactivity is shared with the host SPA —
forking the framework instance would break mounted-component communication.

### Runtime mount contract

`frontend/src/main.ts` exposes a single global via the IIFE name:
...
export const SporaApp = {
    mount(target: HTMLElement, hostContext: { api, pinia, theme, route }) {
        const app = createApp(App, { hostContext })
        app.use(createPinia())
        app.provide('hostApi', hostContext.api)
        app.provide('hostPinia', hostContext.pinia)
        app.provide('hostRoute', hostContext.route)
        app.mount(target)
    },
...
The host SPA calls `mount(target, { api, pinia, theme, route })` when the
user navigates to `/apps/<slug>` and `unmount(target)` when they leave.

### Install-time wiring

The `SporaPluginFrontendInstaller` Composer plugin (in `spora-installer`)
copies the plugin's `frontend/` directory verbatim into
`public/plugins/<slug>/` on `composer install` / `composer update`. The
SPA's dev server proxies `/plugins/<slug>` to the plugin's local Vite
server when one is running — see `spora-frontend/vite.config.ts`'s
`server.proxy` block for the opt-in dev-mode escape hatch.

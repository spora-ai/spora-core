...
Additional auth endpoints (registration, password change, email verification, etc.) are listed in `docs/04_api.md` and registered in `app/Core/routes.php:44-55`.
...
Plugins are not sandboxed — they run as ordinary PHP code with full access to the application, the database, the file system, and any decrypted credentials. The plugin trust model and lifecycle are documented in [07_plugins.md](07_plugins.md) under the Security section.

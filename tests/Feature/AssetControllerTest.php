 ...
 */
function assetTestSetup(bool $asAdmin = true, ?int $userId = null): array
{
    ...
    // `$userId` is honoured in both branches so the caller can pin a
    // specific non-admin identity (e.g. an uploader) without having to
    // rebuild the auth mock.
    $effectiveUserId = $userId ?? ($asAdmin ? 1 : null);
    $auth = Mockery::mock(AuthService::class);
    $auth->allows('isAdmin')->andReturn($asAdmin);
    $auth->allows('currentUserId')->andReturn($effectiveUserId);
...
test('GET /api/v1/assets/{uuid} lets a non-admin uploader fetch their own upload (no task_id)', function (): void {
    // Regression: uploads via MediaUploadController set `user_id` but no
    // `task_id`. The old canAccessAsset() only walked through the task,
    // so every non-admin uploader got a 404 on their own media. The fix
    // mirrors the listing query's ownership union (user_id OR agent_id),
    // which makes the asset bytes reachable for direct uploads.
    $userId = \bootAuthLayer()->register('uploader@example.com', 'Password1!', 'Uploader');
...
test('GET /api/v1/assets/{uuid} lets a non-admin user fetch media produced by an agent they own', function (): void {
    // The second arm of the ownership union: media generated by an agent
    // (no direct `user_id` on the row) is owned by the user via the
    // agent's `user_id`. Even if the asset has no `task_id`, the agent
    // link is enough.
    $userId = \bootAuthLayer()->register('agent-owner@example.com', 'Password1!', 'Agent Owner');
    [$router, $archive, $tmp, $restore] = assetTestSetup(asAdmin: false, userId: $userId);
